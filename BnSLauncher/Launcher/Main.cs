        ...
        public Main()
        {
            ...
            switch (sSettings.sRegion)
            {
                case "KR":
                    ...
                    {
                        ClientFound = false;
                        ClientRegion = "BnS Korea";
                    }
                    if (sSettings.gKorTestPath == false)
                    {
                        ClientFound = false;
                        ClientRegion = "BnS Korea(test)";
                    }
                    break;
                case "EN":
            ...
            {
                DialogResult dr = MessageBox.Show(ClientRegion + msb_clinotfound, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                if (dr == DialogResult.OK)
                {
                    Settings settings = new Settings();
                    settings.FormClosing += new FormClosingEventHandler(Settings_FormClosing);
